...
public class Tester {
    ...
    /**
     * Creates a new Rectangle2D that is grown by delta in each direction compared
     * to the given Rectangle2D.
     *
    ...
    /**
     * Test whether each step of the solution is strictly less than primitive step
     * size
     * 
        ...
        for (int i = 1; i < ps.getRobotPath().size(); i++) {
            ...
            default: {
                ...
                if (direction == -1) {
                    ...
                    System.out.println("Object not moving with robot" + " or pushed to wrong direction at step " + i);
                    pass = false;
                }
            }
            }
    ...
    /**
     * Return the index of the pushed object from given previous state and current
     * state
     * 
     ...
     * @return -2 if multiple moved object found, -1 if no moved object found, index
     *         of moved object otherwise.
     */
    ...
    /**
     * Check if a given robot and a given movable object are coupled together (ready
     * to push)
     * 
     * @param r robot state
     * @param b movable object
     * @return -1 if not coupled, 1 if robot on bottom of box 2 if robot on left of
     *         box 3 if robot on top of box 4 if robot on right of box
     */
    public int isCoupled(RobotConfig r, Box b) {
        ...
        boolean horizontal;
        if ((angle >= Math.PI * 2 && angle <= Math.PI * 2 + angleError)
                || angle < Math.PI * 4 && angle >= Math.PI * 4 - angleError) {
            p1 = getPoint1(r);
            p2 = getPoint2(r);
            horizontal = true;
